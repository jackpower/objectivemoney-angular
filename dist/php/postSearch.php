<?php
    ...
    $sql = "INSERT INTO user_search (userid, pid, value, monthlysalary, lumpsum, addresslat, addresslong, investments, LT, ST, EP, TP, MA, english, afrikaans, zulu, xhosa, southernsotho, tswana, northernsotho, tsonga, venda, swati, ndebele, searchgender, ip) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    // use prepared statements, even if not strictly required is good practice
    $stmt = $pdo->prepare( $sql );

    // execute the query
    $stmt->execute(array($userid, $pid, $value, $monthlysalary, $lumpsum, $addresslat, $addresslong, $investments, $LT, $ST, $EP, $TP, $MA, $english, $afrikaans, $zulu, $xhosa, $southernsotho, $tswana, $northernsotho, $tsonga, $venda, $swati, $ndebele, $searchgender, $ipaddress));
